...
/**
 * A subclass of Parser, this class parses inheritance, namely the 'extends' and 'override' attributes (which go in the &lt;html&gt; element and any element and contain the path to the file to extend and the id of the element to override respectively).
 * 
 * Extension works like this: when a file extends another file, all the elements (but not body text) are copied from the parent file to the child file.  The exception is when one of the child elements overrides one of the parent elements.  In that case, the parent element is not copied and the child element is left unchanged.
 * 
 * @see Parser
 * */
public class ExtendParser extends Parser {
	...
	private NamedNode parse(NamedNode input){
		if(!((Document) input.code).child(0).hasAttr(Language.EXTEND_ATTRIBUTE)) return input; //If we don't actually extend anything here, don't bother.
		NamedNode sup = new NamedNode(resolveRelativePath(input, ((Document) input.code).child(0).attr(Language.EXTEND_ATTRIBUTE))); //Get the NamedNode we extend.
		...
		if(!parsedNodes.containsKey(sup.path)){ //Parse the NamedNode we extend in case it extends something itself.
			sup = parse(sup);
	...
	private Element extend(Element sup, Element sub){
		//Get the child elements for both the sup (super) element and the sub (extended) element.
		...
		//Add the elements from the sup to the beginning of sub.  This is where the real extension happens.
