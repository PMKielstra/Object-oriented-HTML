...
/**
 * A subclass of Parser, this class parses 'blocks', namely, the 'expose' element and the 'use' tag.  These elements are used as follows:
 * <br><br>
 * File a.oohtml:
 * &lt;html&gt;
 * ...
 * &lt;div expose="blockName"&gt;DIVCONTENT&lt;/div&gt;
 * ...
 * &lt;/html&gt;
 * <br><br>
 * File b.oohtml:
 * &lt;html&gt;
 * ...
 * &lt;use name="relative/path/to/a.oohtml/blockName" /&gt;
 * ...
 * &lt;/html&gt;
 * <br><br>
 * When put through the compiler, file a.oohtml is unchanged, but the &lt;use name="relative/path/to/a.oohtml/blockName" /&gt; in file b.oohtml is replaced with &lt;div&gt;DIVCONTENT&lt;/div&gt;.  (Note that the 'expose' attribute is removed.)
 * 
 * 
 * @see Parser
 * */
public class BlockParser extends Parser {
	 ...
	 * @param input A NamedNode, preferably one built using the constructor that takes a path.
	 * @return A NamedNode containing a JSoup document with all blocks resolved.
	 * */
	@Override
	public NamedNode parseDocument(NamedNode input){
		//We need two methods because we need to reset usedPaths when we start to parse a new document and its whole extension tree, but not just when we start to parse a new document.
	...
	private NamedNode parse(NamedNode input){
		...
		for(Element e : elements){
			...
			}else usedPaths.add(nameAndPath);
			//Split up the path around occurrences of either \ or /.
			String[] segments = nameAndPath.split("\\\\|/");
			//The last item in segments is the block's name.  The other items form the path.
			String name = segments[segments.length - 1];
			String path = "";
			//Join all the segments except the last one and put a file separator between them.
			for(int i = 0; i < segments.length - 1; i++) path += (segments[i] + File.separator);
			//The final segment will have had a file separator appended.  Remove it.
			path = path.substring(0, path.length() - File.separator.length());
			//Get the canonical path to the node in question.
			String resolvedPath = resolveRelativePath(input, path);
			//Search through the list of already-parsed nodes.
			NamedNode nn = null;
			...
			//If there are too many blocks with the right name, throw an exception.
			}else if(blocks.size() > 1){
				throw new BadCodeException("There are " + blocks.size() + " blocks called " + name + " in " + nn.path + ".  There should only be one.");
			}
			//Clone the block (because otherwise the call to replaceWith would remove block from nn), remove the 'expose' attribute from it, and insert it into the document.
